...
def test_normal_title_gets_normal_score() -> None:
    ...
    assert candidates[0].confidence == pytest.approx(1.0 * 1.1 * 1.1)


def test_short_title_penalty() -> None:
    candidates = _scored([_candidate(display_title="短")])
    assert candidates[0].confidence == pytest.approx(1.0 * 0.8 * 1.1)


def test_long_title_penalty() -> None:
    candidates = _scored([_candidate(display_title="长" * 101)])
    assert candidates[0].confidence == pytest.approx(1.0 * 0.9 * 1.1)
...
def test_no_leading_word_penalty_for_chinese_numeral_without_第() -> None:
    ...
    assert candidates[0].confidence == pytest.approx(1.0 * 1.1 * 1.1 * 0.5)
...
def test_filler_prefix_penalty() -> None:
    ...
    assert candidates[0].confidence == pytest.approx(1.0 * 1.1 * 1.1 * 0.4)
...
def test_number_increment_bonus() -> None:
    ...
    assert second.confidence == pytest.approx(1.0 * 1.1 * 1.1 * 1.1)
...
def test_number_decrement_penalty() -> None:
    ...
    _scored([first, second])
    assert second.confidence == pytest.approx(1.0 * 1.1 * 1.1 * 0.8)
...
def test_even_density_bonus() -> None:
    ...
    assert scored[0].confidence == pytest.approx(1.0 * 1.1 * 1.1 * 1.2)
    assert scored[1].confidence == pytest.approx(1.0 * 1.1 * 1.1 * 1.1 * 1.2)
    assert scored[2].confidence == pytest.approx(1.0 * 1.1 * 1.1 * 1.1 * 1.2)
...
def test_irregular_density_penalty() -> None:
    ...
    scored = _scored(candidates)
    assert scored[0].confidence == pytest.approx(1.0 * 1.1 * 1.1 * 0.8)
...
def test_fewer_than_three_same_level_no_density_factor() -> None:
    ...
    scored = _scored(candidates)
    assert scored[1].confidence == pytest.approx(1.0 * 1.1 * 1.1 * 1.1)
