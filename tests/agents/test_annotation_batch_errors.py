...
实体引用统一 EntityRef（编号 n 或本 chunk 的 el 键）、收尾更名 finish_chapter。
...
def test_relation_endpoint_rejections_are_per_record_and_localized() -> None:
    ...
    2026-09-14 参数换算：两端改 EntityRef（from_ref/to_ref），拒绝文案的标签前缀
    由旧 "write_relation.<field>" 变为记录键 "<record>.<field>"；expected 仍指向
    write_entity 编号来源。等价意图=每条违规关系都拿到指向自己的可自纠错误，
    且任何一条失败都不产生落账记录。
    """
    ledger = _ledger()
    ...
    for rejection, number in zip(rejections, (1, 3), strict=True):
        ...
        assert f"实体编号 {number} 未登记" in str(rejection)
...
def test_event_participant_rejections_are_per_record_and_localized() -> None:
    ...
    数组并入 apply_event），未登记编号在写入点即拒绝，
    record=树/节点/参与者编号（entityid 原值）。等价意图=每个非法参与者都有自己的错误，
...
def test_dialogue_speaker_rejection_points_at_the_single_record() -> None:
    ...
    （record=dialogue/<序号>、field=speaker）。2026-09-14 speaker 参数为 EntityRef
    （编号 n 或 el 键，账本形参 speaker_ref）。等价意图=错误定位到唯一可修记录，
...
def test_dialogue_coverage_missing_candidates_defaults_to_not_dialogue() -> None:
    ...
    finish_chapter（ledger.chunk_finished→chapter_finished），因此本测试直接走
