...
def _insert_chunks(db_session, run_id: str, chunks: list[Chunk]) -> None:
...
def test_insert_and_fetch_paragraphs(db_session) -> None:
    ...
    _insert_chunks(db_session, run_id, chunks)
    ...
    for i, (row, span) in enumerate(zip(rows, spans, strict=True)):
        assert row.chunk_id == span.chunk_id == 0
...
def test_get_incomplete_paragraph_chunk_ids(db_session) -> None:
    """缺段落的 chunk 被列出（空文本 chunk 除外），补全后为空"""
    run_id = _create_run(db_session)
    text_a = "甲段。\n乙段。\n"
    text_b = "丙段。\n"
    chunks = [
        Chunk(index=0, text=text_a, start=0, end=len(text_a), chapter_id=1),
        Chunk(index=1, text=text_b, start=len(text_a), end=len(text_a) + len(text_b), chapter_id=1),
        Chunk(index=2, text="", start=len(text_a) + len(text_b), end=len(text_a) + len(text_b), chapter_id=1),
    ]
    _insert_chunks(db_session, run_id, chunks)
    all_spans = _make_spans(chunks[:2])

    repo = ParagraphRepository(db_session)
    # 只插入 chunk 0 的段落 → chunk 1 缺失，chunk 2 空文本被排除
    chunk0_spans = [s for s in all_spans if s.chunk_id == 0]
    repo.insert_paragraphs(run_id, chunk0_spans)
    assert repo.get_incomplete_paragraph_chunk_ids(run_id) == [1]

    # 补全后为空
    repo.insert_paragraphs(run_id, all_spans)
    assert repo.get_incomplete_paragraph_chunk_ids(run_id) == []


def test_get_incomplete_paragraph_chunk_ids_detects_gaps(db_session) -> None:
    """chunk 内段落序号不连续（min != 0 或 count != max + 1）被列出"""
    run_id = _create_run(db_session)
    ...
    ParagraphRepository(db_session).insert_paragraphs(run_id, spans)
    assert ParagraphRepository(db_session).get_incomplete_paragraph_chunk_ids(run_id) == [0]
