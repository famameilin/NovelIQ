...
def test_apply_reference_resolutions_to_history_clears_removed_reference_resolutions(db_session) -> None:
    ...
    创建时间: 2026-04-29
    任务: 角色引用分层重构
    新建原因: 当前状态里已移除的 reference resolution 必须把历史行上的旧 resolved 字段撤销干净。
    """
    novel_id = uuid.uuid4().hex[:8]
    _insert_test_novel(db_session, novel_id)
    run_id = RunRepository(db_session).create_run(
        novel_id=novel_id,
        source_path="test",
        title="Reference Resolution Demotion",
    )
    ChunkRepository(db_session).insert_chunks(run_id, [Chunk(index=1, text="我看向白芷。", start=0, end=100)])
    ...
    assert report["chunk_characters"] == 1
    assert report["chunk_dialogues"] == 1
    assert report["chunk_relations"] == 1
    assert chunk_character.resolved_global_name is None
    assert chunk_character.global_skip_reason == "unresolved pov reference"
    assert chunk_dialogue.speaker_references[0]["resolved_global_name"] is None
    assert chunk_dialogue.speaker_references[0]["global_skip_reason"] == "unresolved pov reference"
