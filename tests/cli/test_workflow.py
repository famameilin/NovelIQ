...
class TestWorkflowHelpers:
    ...
    def test_check_stage_completion_preprocess(self, db_session):
        """测试 preprocess 阶段完成检查"""
        from src.cli.workflow_helpers import _check_stage_completion

        novel_id = f"test_novel_{uuid.uuid4().hex[:8]}"
        run_repo = RunRepository(db_session)
        run_id = run_repo.create_run(novel_id=novel_id, source_path="test", title="Test Novel")

        assert not _check_stage_completion(db_session, "preprocess", run_id)
        ...
        assert _check_stage_completion(db_session, "preprocess", run_id)

    def test_check_stage_completion_annotate(self, db_session):
        """测试 annotate 阶段完成检查"""
        from src.cli.workflow_helpers import _check_stage_completion
        ...
        assert not _check_stage_completion(db_session, "annotate", run_id)
        ...
        assert _check_stage_completion(db_session, "annotate", run_id)

    def test_check_stage_completion_aggregate(self, db_session):
        """测试 aggregate 阶段完成检查"""
        from src.cli.workflow_helpers import _check_stage_completion
        ...
        assert not _check_stage_completion(db_session, "aggregate", run_id)
        ...
        assert _check_stage_completion(db_session, "aggregate", run_id)
