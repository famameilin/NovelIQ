...
describe("useSSEListener", () => {
  ...
  it("url 变化（taskId 切换）重建连接时不携带旧 last_seq（P1-8 跨任务串流修复）", () => {
    ...
    expect(MockEventSource.instances[1].url).toBe("http://test/stream?task_id=task-b");
  });
  ...
  it("同一任务下 url 重建（effect 重建）保留 last_seq，query 只回放增量（P1-1 修复）", () => {
    ...
    // 避免 LLM 输出重复、进度回退
    view.rerender(<Harness url="http://test/stream?task_id=task-a&view=graph" />);

    expect(MockEventSource.instances).toHaveLength(2);
    expect(MockEventSource.instances[0].closed).toBe(true);
    expect(MockEventSource.instances[1].url).toBe(
      "http://test/stream?task_id=task-a&view=graph&last_seq=42"
    );
  });
  ...
  it("监听启停（url 变 null 再恢复同一任务）保留 last_seq（P1-1 修复）", () => {
    ...
    view.rerender(<Harness url="http://test/stream?task_id=task-a" />);
    expect(MockEventSource.instances).toHaveLength(2);
    expect(MockEventSource.instances[1].url).toBe(
