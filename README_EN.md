...
The system accepts `.txt` novel files and automatically handles encoding detection, text cleaning, word segmentation and chapter-first chunking. A six-stage pipeline (Preprocessing → Annotation → Linguistic Analysis → Aggregation → Topic Modeling → Diagnosis) produces analysis results, with each stage independently persisted and resumable.
...
The linguistic stage enables LTP and Word2Vec by default and never downloads models at runtime. Prepare these assets before starting an analysis:

- `models/ltp/small/`: a complete offline LTP model directory containing its configuration, vocabulary, and weights
- `models/word2vec/shared/`: exactly one converted `.kv` file plus any same-name array sidecar generated by Gensim

The `models/` directory is excluded from Git. Docker Compose mounts the host directory at `/app/models`. After installing source dependencies, an existing `.vec`, `.bin`, or `.txt` pretrained vector file can be converted with:

```powershell
uv run python -m scripts.tools.convert_word2vec_pretrained
```

The converter reads from `models/word2vec/pretrained/` and writes to `models/word2vec/shared/` by default.
...
1. Configure environment variables
   ...
   # Edit .env.docker to set model API endpoint, keys, and LTP_MODEL_DIR
   ```
...
│  linguistic analysis, aggregation, topics, diagnosis    │
...
flowchart LR
    ...
    C --- C1[LTP syntax and entities\nFixed phrases\nWord2Vec]
    D --- D1[Sentiment & pacing curves\nGlobal statistics\nQuality gate]
    E --- E1[LDA topic inference\nModel persistence]
    F --- F1[Cloud LLM diagnosis\nDiagnostic report]
```
...
| **Linguistic Analysis** | `run_linguistic` | LTP syntax and entities, fixed phrases, and Word2Vec POS vectors |
