...
修改内容: 将所有方法改为 async def，直接 await workflow 函数
"""
...
class StageExecutor:
    ...
    async def run_aggregate(
        ...
        await run_aggregate(run_id=run_id, session=session, notify_callback=notify_callback)
    ...
    async def run_topic_model(
        ...
        await run_topic_model(run_id=run_id, session=session, num_topics=num_topics, notify_callback=notify_callback)
