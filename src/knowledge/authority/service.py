...
from src.storage.repositories import DiagnosisRepository, GraphRepository
...
class KnowledgeGraphAuthorityService:
    ...
    def build_graph_view(self, run_id: str, event_limit: int | None = 200) -> GraphAuthorityView:
        ...
        entities = self._graph_repo.fetch_entities(run_id)
        summary = self._diagnosis_repo.fetch_graph_summary(run_id) if self._diagnosis_repo is not None else {}
        ...
        return GraphAuthorityView(
            ...
            relation_events=self._build_relation_events(
                self._graph_repo.fetch_relation_events(run_id, limit=event_limit)
            ),
        ...
        for entity in sorted(entities, key=lambda row: row.canonical_name):
            canonical_entities.append(
                CanonicalEntity(
                    ...
                    last_emotion_score=entity.last_emotion_score,
