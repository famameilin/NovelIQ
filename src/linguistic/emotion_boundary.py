"""段落级情绪边界（2026-09-07 定稿句级，2026-09-14 改段落级监督，用户定稿）

监督信号 = 标注 agent 逐章自选段的整段情绪标签（write_metrics.labels 随
chapter_annotations 落库）；边界 = 该书自己的段落向量现算的岭回归线性映射
（paragraph_embeddings 向量 → 情绪分值），numpy 亚秒级、确定性；
无标签 run 无边界（上层保留词典/mNEG 口径）。

拟合器对"文本向量 → 分值"这一抽象建模（不绑定句/段粒度）：向量源、样本单位
由上层决定，算法（中心化岭回归闭式解 + 截距）与 09-07 版一致。

分值目标 = 标签自带的 -2..2 整数分值（强烈负面=-2 … 强烈正面=2）；
p(负向) 由分值单调映射供分层（p≥0.8 强），跨书曲线口径不同不可比（用户已接受）。

纯函数，不依赖模型实例；测试可直接构造向量矩阵。
"""
...
#: 岭正则强度：向量已 L2 归一化、样本量小（每章 2-3 段），固定值保证数值稳定
_RIDGE_LAMBDA = 1.0
#: 强度分层阈值（方案：p≥0.8 强）
_STRONG_SCORE_THRESHOLD = 0.8
#: 向量最大维度上限（防御异常模型输出；ritrieve 段落向量 1024）
_MAX_DIM = 4096


@dataclass(frozen=True)
class EmotionBoundary:
    """按书情绪边界：权重向量与截距（负向分值方向）"""
    ...
    def score(self, vector: np.ndarray) -> float:
        """单条分值：越高越正，越低越负（0 附近为中性带）"""
        return float(np.dot(vector, self.weights) + self.intercept)


def fit_emotion_boundary(
    vectors: np.ndarray,
    emotions: list[int],
) -> EmotionBoundary | None:
    """用全书自选段标签拟合线性边界（岭回归闭式解）

    vectors: (N, D) 文本向量（建议 L2 归一化）；emotions: 长度 N 的 -2..2 整数分值。
...
def score_items(
    boundary: EmotionBoundary,
    vectors: np.ndarray,
) -> list[float | None]:
    """逐条打分：返回情绪分值列表（维度不匹配按 None 处理，不伪造）"""
    ...
    for vector in vectors:
        if vector.shape[0] != expected_dim:
            scores.append(None)
            continue
        scores.append(boundary.score(vector))
    return scores
