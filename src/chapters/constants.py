...
# 开篇标题猜测：楔子/序章/前言 等（不含 EXTRA_RE 的章节类词）
# 2026-08-13 P2：标题形态约束——关键词后仅允许分隔符/空白/短标题内容，
# 剩余部分不得含句末标点，防止正文段落首行（如「引言：本书讲述了……」）被误当开篇标题
PRELIMINARY_TITLE_RE = re.compile(
    r"^(楔子|序言|前言|引子|序章|序幕|引言|写在前面|Prologue)"
    r"(?:[ \t]*[：:、.\-—][ \t]*)?([^\n。！？…]*)$",
