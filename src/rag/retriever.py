...
class DisambigContextProvider:
    ...
    def build_disambiguation_hint(self, existing_names: list[str]) -> str | None:
        """构建消歧专用的图谱反馈提示。

        将图谱已裁决的别名映射和已确认关系追加到 hint 中，
        让消歧 LLM 能看到当前已解析的实体信息。

        此方法封装了图谱权威数据的消费逻辑，调用方无需了解内部实现。

        创建时间: 2026-04-17
        创建者: TraeAI
        任务: refactor/split-provider-bundle-renderer
        说明: 作为 _augment_hint_with_graph 的公共接口，消除私有属性访问
        """
        if self._graph_repo is None or self._run_id is None:
            return None
