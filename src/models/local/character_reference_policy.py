...
def build_reference_slot(surface_name: str | None, *, chapter_id: int | None = None) -> str | None:
    ...
    chunk_part = f"_C{chapter_id}" if chapter_id is not None else ""
...
def collect_reference_slots_from_names(
    ...
    *,
    ...
    for name in names:
        slot = build_reference_slot(name, chapter_id=chapter_id)
...
def decide_character_reference(
    ...
    *,
    ...
    return CharacterReferenceDecision(
        ...
        reference_slot=build_reference_slot(normalized, chapter_id=chapter_id),
