...
def metaphor_density(text: str) -> float:
    ...
    if not sentences:
        return 0.0
    markers = ("像", "如", "似", "般", "宛若", "犹如")
    hit = sum(1 for sentence in sentences if any(m in sentence for m in markers))
    return hit / len(sentences)


def lexicon_density(tokens: Sequence[str], terms: Iterable[str]) -> float:
    return count_token_hits(tokens, terms) / max(len(tokens), 1)


def sensory_density(text: str, terms: Iterable[str]) -> float:
    tokens = tokenize_words(text)
    return lexicon_density(tokens, terms)
...
def semantic_category_densities(text: str, category_terms: Dict[str, List[str]]) -> Dict[str, float]:
    ...
    for category, terms in category_terms.items():
        ...
        densities[category] = hit_count / total_tokens
    return densities
