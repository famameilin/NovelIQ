...
- `DATABASE`：开发数据库，包含 `url`、`username`、`password`
- `TEST_DATABASE`：测试数据库，包含 `url`、`username`、`password`
- `MODEL`：文本模型服务，包含 `base_url`、`model`、`api_key`
- `EMBEDDING_MODEL`：Embedding 模型服务，包含 `base_url`、`model`、`api_key`

数据库对象中的 `url` 只填写协议、主机、端口和数据库名，账号密码使用独立属性。文本标注、标注兜底和诊断任务共用 `MODEL`。完整格式参考 `.env.example` 和 `.env.docker.example`。
...
- 标注主模型发生 `AnnotationAgentRunError` 时，启用标注兜底后以同一 chunk、身份记忆和证据服务重试
- 文本标注、标注兜底和诊断使用同一文本模型连接；Embedding 使用独立连接
